  ...
  return (
    <div style={{ padding: 16 }}>
      <Descriptions title="General" bordered column={1} size="small">
        ...
        <Descriptions.Item label="Status Code">{request.statusCode} {request.statusText}</Descriptions.Item>
        {request.remoteIP && (
      ...
      <Descriptions title="Timing" bordered column={1} size="small" style={{ marginTop: 16 }}>
        {request.timing.queueing > 0 && (
          <Descriptions.Item label="Queueing">{request.timing.queueing.toFixed(2)} ms</Descriptions.Item>
        )}
        {request.timing.dnsLookup > 0 && (
          <Descriptions.Item label="DNS Lookup">{request.timing.dnsLookup.toFixed(2)} ms</Descriptions.Item>
        )}
        {request.timing.initialConnection > 0 && (
          <Descriptions.Item label="Initial Connection">{request.timing.initialConnection.toFixed(2)} ms</Descriptions.Item>
        )}
        {request.timing.sslHandshake > 0 && (
          <Descriptions.Item label="SSL/TLS">{request.timing.sslHandshake.toFixed(2)} ms</Descriptions.Item>
        )}
        {request.timing.requestSent > 0 && (
          <Descriptions.Item label="Request Sent">{request.timing.requestSent.toFixed(2)} ms</Descriptions.Item>
        )}
        {request.timing.waiting > 0 && (
          <Descriptions.Item label="Waiting (TTFB)">{request.timing.waiting.toFixed(2)} ms</Descriptions.Item>
        )}
        {request.timing.contentDownload > 0 && (
          <Descriptions.Item label="Content Download">{request.timing.contentDownload.toFixed(2)} ms</Descriptions.Item>
        )}
      ...
      <Descriptions title="Size" bordered column={1} size="small" style={{ marginTop: 16 }}>
        <Descriptions.Item label="Request Size">{formatFileSize(request.requestSize)}</Descriptions.Item>
        <Descriptions.Item label="Response Size">{formatFileSize(request.responseSize)}</Descriptions.Item>
        <Descriptions.Item label="Total Size">{formatFileSize(request.totalSize)}</Descriptions.Item>
      </Descriptions>
